...
export function NotificationsPopover({ notifications, unread }: { notifications: Notif[]; unread: number }) {
  ...
  return (
    <Dropdown
      width="w-96"
      trigger={
        <button className="relative rounded-md p-2 text-slate-500 transition hover:bg-slate-100" aria-label="Notifications">
      ...
      <div className="max-h-96 overflow-y-auto scrollbar-thin">
        ...
        {items.slice(0, 8).map((n) => (
          <button
            ...
            onClick={() => openNotif(n)}
            className={cn(
              "flex w-full items-start gap-3 border-b border-slate-50 px-4 py-3 text-left transition last:border-0 hover:bg-slate-50",
