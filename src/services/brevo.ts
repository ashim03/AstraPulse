...
function otpBlock(code: string, minutes: number): string {
  return `
<p style="margin:0 0 16px;color:#4b5563;font-size:15px;line-height:1.6;">Use the verification code below to continue:</p>
<table width="100%" cellpadding="0" cellspacing="0"><tr><td align="center">
  <div style="background:#f0f0ff;border:2px dashed #6366f1;border-radius:8px;padding:20px 32px;margin:16px 0;">
    <span style="font-size:32px;font-weight:700;color:#6366f1;letter-spacing:6px;font-family:'Courier New',monospace;">${code}</span>
  </div>
</td></tr></table>
<p style="margin:12px 0 0;color:#6b7280;font-size:13px;">This code expires in <strong>${minutes} minute${minutes !== 1 ? "s" : ""}</strong>. If you did not request this, please ignore this email.</p>`;
}
...
export async function sendOtpEmail(
  ...
  const subjectMap: Record<string, string> = {
    registration: "Verify Your AstraPulse Account",
    login: "Your AstraPulse Login Code",
    password_reset: "Your AstraPulse Password Reset Code",
  };
  ...
  const html = wrapTemplate(
    ...
    `<p style="margin:0 0 16px;color:#4b5563;font-size:15px;line-height:1.6;">${greeting}</p>
     ${otpBlock(code, 5)}
     <p style="margin:24px 0 0;color:#9ca3af;font-size:12px;">If you did not request this code, please secure your account immediately.</p>`
  );
...
export async function sendVerificationEmail(
  ...
  const html = wrapTemplate(
    ...
    `<p style="margin:0 0 16px;color:#4b5563;font-size:15px;line-height:1.6;">${greeting}</p>
     <p style="margin:0 0 24px;color:#4b5563;font-size:15px;line-height:1.6;">Thank you for signing up for AstraPulse. Please verify your email address by clicking the button below:</p>
     ${brandButton(verifyUrl, "Verify Email Address")}
     <p style="margin:24px 0 0;color:#6b7280;font-size:13px;">If the button above doesn't work, copy and paste this URL into your browser:<br><a href="${verifyUrl}" style="color:#6366f1;word-break:break-all;">${verifyUrl}</a></p>`
  );

  const result = await sendEmail(workspaceId, {
    to: [{ email: to, name }],
    subject: "Verify Your Email Address — AstraPulse",
    htmlContent: html,
  });
  return { success: result.success, error: result.error };
}
